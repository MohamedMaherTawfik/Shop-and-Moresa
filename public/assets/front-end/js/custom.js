...
// Debug: Check if checkbox exists when page loads
$(document).ready(function() {
    console.log('Checkbox exists:', $("#inputChecked").length);
    console.log('Sign up button exists:', $("#sign-up").length);
    console.log('Sign up button disabled:', $("#sign-up").prop('disabled'));
});
...
$("#customer-register-form").on("submit", function (e) {
    ...
    if ($("#sign-up").prop('disabled')) {
        ...
        if (typeof toastr !== 'undefined') {
            toastr.error('يرجى الموافقة على الشروط والأحكام أولاً');
        } else {
    ...
    if (typeof $.ajax === 'undefined') {
        console.log('AJAX not available, using fallback submission');
        this.submit();
        return false;
    }
    ...
    $.ajax({
        ...
        beforeSend: function (xhr) {
            ...
            console.log('Request URL:', $(this).data("action"));
            console.log('Request data:', $(this).serialize());
            $("#loading").addClass("d-grid");
        },
        success: function (response) {
            ...
            if (response.errors) {
                ...
                console.log('Redirecting to verification:', response.redirect_url);
                window.location.href = response.redirect_url;
            } else {
                ...
                // Check if it's a successful response without status
                if (response.message && response.message.includes('successful')) {
                    ...
                    console.log('Unknown response format, showing generic success');
                    if (typeof toastr !== 'undefined') {
                        toastr.success('تم التسجيل بنجاح!');
                    } else {
                        alert('تم التسجيل بنجاح!');
        ...
        error: function (xhr, status, error) {
            ...
            if (xhr.status === 419) {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error('انتهت صلاحية الجلسة. يرجى تحديث الصفحة والمحاولة مرة أخرى.');
                } else {
                    ...
                    if (response.errors) {
                        for (let index = 0; index < response.errors.length; index++) {
                            if (typeof toastr !== 'undefined') {
                                ...
                                alert(response.errors[index].message);
                            }
                        }
                    } else {
                        if (typeof toastr !== 'undefined') {
                            toastr.error('خطأ في البيانات المدخلة. يرجى التحقق من المعلومات.');
                        } else {
                            alert('خطأ في البيانات المدخلة. يرجى التحقق من المعلومات.');
                        }
                    }
                } catch (e) {
                    if (typeof toastr !== 'undefined') {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error('خطأ في الخادم. يرجى المحاولة لاحقاً.');
                } else {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error('انتهت مهلة الطلب. يرجى المحاولة مرة أخرى.');
                } else {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error('حدث خطأ في التسجيل. يرجى المحاولة مرة أخرى.');
                } else {
