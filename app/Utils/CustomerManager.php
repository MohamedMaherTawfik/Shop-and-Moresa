...
{
    try {
        // تحقق أولاً إذا كانت خاصية المحفظة مفعّلة
        $walletStatus = BusinessSetting::where('type', 'wallet_status')->first();
        ...
        // تحديد نوع العملية (إضافة رصيد أو خصم)
        if (in_array($transaction_type, ['add_fund_by_admin', 'add_fund', 'order_refund', 'loyalty_point', 'coupon_redeem'])) {
            $credit = $amount;

            if ($transaction_type == 'add_fund') {
                $bonus = Helpers::add_fund_to_wallet_bonus(Convert::usd($amount ?? 0));
                $wallet_transaction->admin_bonus = $bonus;
                $add_fund_to_wallet_bonus = $bonus;
            } elseif ($transaction_type == 'loyalty_point') {
                $exchange_rate = BusinessSetting::where('type', 'loyalty_point_exchange_rate')->first()->value ?? 1;
                $credit = (($amount / $exchange_rate) * Convert::default(1));
            }
        ...
        // ✅ هنا التعديل الأساسي: نرجّع الكائن نفسه وليس true
        return $wallet_transaction;

    } catch (\Exception $ex) {
        DB::rollBack();
        info('create_wallet_transaction error: ' . $ex->getMessage());
        return false;
    }
}
